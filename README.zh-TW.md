...
### Predict *(需要登入)*
從 32 強一路預測到決賽的每場淘汰賽勝隊。預測會自動往後傳遞，每場比賽在開球時鎖定，只有真實比賽結束後才計分。提前確認完整預測表可獲 1.4 倍加成。

![Predict](docs/screenshots/Predict_tw.png)

### Fantasy *(需要登入)*
每輪組建 5 人陣容（GK、DEF、MID、FWD、FLEX）。支援隊長、轉會、國家名額限制、淘汰隊伍處理、球員照片及重點球員標記。

![Fantasy](docs/screenshots/Fantasy_tw.png)

### 球隊（Team）
瀏覽所有國家隊——主隊風格與吉祥獸、近況、下一場比賽、陣容、重點球員，以及根據真實賽事數據建立的能力雷達圖。

![Team](docs/screenshots/Team_tw.png)

### 賽程（Schedule）
小組積分榜、賽程表、即時比分、晉級狀態，以及已完賽比賽含進球時序與球隊統計的詳細報告。

![Schedule](docs/screenshots/Schedule_tw.png)

### 聯賽（League）*(需要登入)*
整合 Supabase 的排行榜，並排顯示所有已登入玩家的 Predict 與 Fantasy 積分。

![League](docs/screenshots/League_tw.png)
